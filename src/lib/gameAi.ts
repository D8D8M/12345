...
export const STRANGER_BETRAYAL_LINES: [string, string] = [
  'Ха-ха-ха! Как же легко тобой манипулировать. Я нашел тебя беспомощным и использовал как живой таран, чтобы твоими руками зачистить замок и устранить Помощника!',
...
export const requestStrangerBetrayalLines = (run: RunSummary) => Promise.all([
  askGameAi(`Незнакомец только что пронзил Помощника теневым клинком. Перефразируй эту реплику, сохранив манипуляцию, живой таран, зачистку замка и устранение Помощника: «${STRANGER_BETRAYAL_LINES[0]}». Данные пути: ${facts(run)}`, STRANGER_BETRAYAL_LINES[0]),
