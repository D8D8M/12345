...
const askGameAi = async (prompt: string, fallback: string): Promise<string> => {
  try {
    const { data, error } = await supabase.functions.invoke('ai', {
      body: {
        prompt,
        system: 'Ты внутриигровой рассказчик мрачной русскоязычной игры «Fallen Knight». Отвечай только запрошенным текстом, без markdown, безопасно и не более 35 слов.',
      },
    });
