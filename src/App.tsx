  ...
  return (
    <main className="h-screen h-dvh overflow-hidden bg-[#090e12] font-sans text-slate-100 selection:bg-teal-300/30">
      ...
      {!started && !choosingMode && <div ref={menuSceneRef} onMouseMove={moveMenuParallax} className="dead-menu fixed inset-0 z-[90] overflow-hidden bg-[#10152d] text-slate-100">
        ...
        <header className="absolute inset-x-0 top-0 z-20 flex items-start justify-between p-6 md:p-10"><div><p className="text-[9px] font-black uppercase tracking-[.48em] text-cyan-200/70">Проклятое королевство</p><h1 className="mt-2 text-2xl font-black uppercase tracking-[.12em] text-slate-100 drop-shadow-[0_3px_8px_rgba(0,0,0,.8)] md:text-4xl">Путь к престолу</h1></div><button onClick={() => session ? supabase.auth.signOut() : setShowAuth(true)} disabled={!authReady} className="dead-menu-account border border-white/15 bg-black/20 px-4 py-2 text-[9px] font-black uppercase tracking-[.18em] text-slate-300">{session ? 'Выйти' : 'Войти'}</button></header>
      ...
      {!started && menuTab === 'achievements' && <div className="fixed inset-x-[5vw] bottom-[10vh] top-[18vh] z-[60] overflow-y-auto border border-amber-300/15 bg-[#0a1015]/98 p-5 text-left shadow-[0_30px_90px_rgba(0,0,0,.8)] backdrop-blur-md md:p-7">
        <div className="sticky top-0 z-10 flex items-end justify-between border-b border-white/10 bg-[#0a1015]/95 pb-3"><div><p className="text-[9px] font-black uppercase tracking-[.35em] text-amber-300">Путь к престолу</p><h2 className="mt-1 text-2xl font-black uppercase">Достижения</h2></div><div className="flex items-center gap-3"><b className="text-xs text-amber-200">{unlockedAchievements.size} / {ACHIEVEMENTS.length}</b><button onClick={() => setMenuTab('play')} className="border border-white/15 px-3 py-2 text-[9px] font-black uppercase text-slate-400 hover:border-amber-300/50 hover:text-amber-200">Закрыть</button></div></div>
      ...
      <div className="relative flex h-full min-h-0 w-full flex-col justify-center">
        ...
        {started && <header className="flex shrink-0 items-end justify-between gap-4 px-4 py-2 md:px-7">
          <div><p className="text-[10px] font-bold uppercase tracking-[.38em] text-teal-300/70">Stage {String(sector).padStart(2, '0')} · {LOCATION_NAMES[location]}</p><h1 className="text-xl font-black uppercase tracking-tight md:text-2xl">Путь к престолу <span className="font-light text-slate-500">// {currentTheme.subtitle}</span></h1></div>
